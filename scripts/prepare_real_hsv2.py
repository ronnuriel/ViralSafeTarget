...
def genbank_to_gff3(genbank_path: Path, output_path: Path) -> str:
    ...
    for feature in record.features:
        ...
        lines.append(
            "\t".join([
                seqid, "NCBI_GenBank", feature.type, str(start), str(end), ".", strand, phase,
                ";".join(attrs),
            ])
        )
...
def build_sample(
    ...
    for record in SeqIO.parse(str(all_genomes_fasta), "fasta"):
        ...
        if not quality_ok(sequence, min_length, max_length, max_n_fraction):
            continue
    ...
    selected.extend(usable[: max(0, sample_size - 1)])
    ...
    return considered, len(selected)
...
def main() -> None:
    ...
    if gff_seqid != reference.id:
        print(f"WARNING: GenBank seqid={gff_seqid}, FASTA reference id={reference.id}. "
              "The generated GFF uses the GenBank seqid; verify consistency.")
    considered, selected = build_sample(
        reference,
